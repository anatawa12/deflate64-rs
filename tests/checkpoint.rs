...
fn build_synthetic_checkpoint(window_data: &[u8]) -> Vec<u8> {
    ...
    cp[0..2].copy_from_slice(&0x1001u16.to_le_bytes()); // version
    let input_bits = 1000u64;
    // For uncompressed blocks, input must be byte-aligned, so round up to next byte boundary
    let byte_aligned_bits = input_bits.div_ceil(8) * 8;
    cp[2..10].copy_from_slice(&byte_aligned_bits.to_le_bytes()); // input_bits
    cp[10] = 0; // buffered_value (no partial bits for byte-aligned position)
    cp[11] = 0; // block_type=Uncompressed, bfinal=0
    cp[12..14].copy_from_slice(&33333u16.to_le_bytes()); // uncompressed_remaining
    ...
    cp[346..346 + window_data.len()].copy_from_slice(window_data); // window_data
    let checksum = fletcher32_checksum(&cp);
    cp.extend_from_slice(&checksum.to_le_bytes());
    cp
}

fn rebuild_checkpoint_checksum(data: &mut [u8]) {
    let covered_len = data.len() - 4;
    let checksum = fletcher32_checksum(&data[..covered_len]);
    data[covered_len..].copy_from_slice(&checksum.to_le_bytes());
}
...
fn failed_restore_preserves_inflater_state() {
    ...
    let len = bad_cp.len();
    bad_cp[11] = 2; // block_type=Dynamic, bfinal=0
    bad_cp[12..14].fill(0); // uncompressed_remaining = 0
...
#[test]
fn reject_bad_checkpoints() {
    let checkpoints = inflate_with_checkpoints(10000);
    let (cp_data, _) = &checkpoints[checkpoints.len() / 2];
    let mut inflater = Box::new(InflaterManaged::new());

    let mut bad_version = cp_data.clone();
    bad_version[0] = 0x11;
    rebuild_checkpoint_checksum(&mut bad_version);
    assert!(inflater.restore_from_checkpoint(&bad_version).is_none());

    let n = cp_data.len();
    for len in (0..=500).chain(n - 10..=n - 1) {
        let truncated = &cp_data[..len];
        assert!(inflater.restore_from_checkpoint(truncated).is_none());
    }

    let mut too_long = cp_data.clone();
    too_long.push(0);
    assert!(inflater.restore_from_checkpoint(&too_long).is_none());
...
fn checkpoint_with_large_unread_buffer() {
    ...
    const DEFLATE64_ZERO_256KB: [u8; 18] = [
        0x63, 0x60, 0x18, 0xfd, 0xff, 0x07, 0xa3, 0xfd, 0xff, 0x60, 0xb4, 0xff, 0x1f, 0x8c, 0xf6, 0xff,
        0x03, 0x00,
    ];
...
fn all_window_bytes_restored_properly() {
    const OUTPUT_BUFFER_SIZE: usize = 131072; // Match actual WINDOW_SIZE
